#ifndef __PX_UART_CFG_H__
#define __PX_UART_CFG_H__

#include "px_defs.h"
#include "px_board.h"

#define PX_UART_CFG_UART1_EN 1
#define PX_UART_CFG_UART2_EN 0
#define PX_UART_CFG_UART4_EN 0
#define PX_UART_CFG_UART5_EN 0
#define PX_UART_CFG_DEFAULT_BAUD 115200ul
#define PX_UART_CFG_UART1_TX_BUF_SIZE 256
#define PX_UART_CFG_UART1_RX_BUF_SIZE 256
#define PX_UART_CFG_UART2_TX_BUF_SIZE 0
#define PX_UART_CFG_UART2_RX_BUF_SIZE 0
#define PX_UART_CFG_UART4_TX_BUF_SIZE 0
#define PX_UART_CFG_UART4_RX_BUF_SIZE 0
#define PX_UART_CFG_UART5_TX_BUF_SIZE 0
#define PX_UART_CFG_UART5_RX_BUF_SIZE 0
#define PX_UART_CFG_UART1_CLK_HZ PX_BOARD_PER_CLK_HZ
#define PX_UART_CFG_UART2_CLK_HZ PX_BOARD_PER_CLK_HZ
#define PX_UART_CFG_UART4_CLK_HZ PX_BOARD_PER_CLK_HZ
#define PX_UART_CFG_UART5_CLK_HZ PX_BOARD_PER_CLK_HZ
#define PX_UART_CFG_ERR_STAT_EN 1

#endif
